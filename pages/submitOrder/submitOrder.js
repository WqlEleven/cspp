...
Page({
  ...
  getSubmitCart: function (cartId) {
    ...
    app.loading()
    wx.request({
      url: app.globalData.config.host + '/home/cart_submit',
      data: {
        cart_id: '[' + cartId + ']',
  ...
  settlement: function () {
    ...
    wx.request({
      url: app.globalData.config.host + '/home/order',
      method: 'post',
      header: { "Content-Type": "application/x-www-form-urlencoded" },
      data: {
      ...
      success: function (res) {
        app.hide()
        if (!res.data.errCode) {
          wx.redirectTo({ url: '/pages/pay/pay?id=' + res.data.order_id })
        } else {
