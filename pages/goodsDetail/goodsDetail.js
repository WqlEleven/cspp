...
Page({
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      ...
      path: '/pages/goodsDetail/goodsDetail?id=' + that.data.goods.id + '&parent_id=' + that.data.shopkeeperInfo.id,
  ...
  buyNow: function (e) {
    ...
    if (this.data.goods.stock >= num) {
      wx.navigateTo({
        url: '/pages/submitOrder/submitOrder?id=' + this.data.current + '&num=' + num + '&type=0'
      })
    } else {
      app.error('库存不足！')
    }
  ...
  addcars() {
    if (this.data.goods.stock >= this.data.num) {
      ...
      var that = this
      wx.request({
        url: app.globalData.config.host + '/home/cart_add',
        data: {
          'session_key': session_key,
